...
export default function SignUp() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-100 via-teal-200 to-green-300"
    style={{
        backgroundImage: "url('/images/bg_image.jpg')",
        backgroundSize: "cover",
        backgroundRepeat: "no-repeat",
      }}>
      <div className="bg-white p-8 rounded-lg shadow-xl w-96 animate__animated animate__fadeIn"
      style={{
        ...
        <form onSubmit={handleSubmit} className="space-y-4 animate__animated animate__slideInRight animate__delay-1s">
          <div>
          ...
          <div>
            <label htmlFor="cnfPassword" className="block text-gray-700 text-sm font-bold mb-2">
              Confirm Password:
            </label>
            <input
              type="password"
          ...
          >
            {isLoading ? <div className="animate-spin h-5 w-5 mx-auto border-t-2 border-b-2 border-white rounded-full"></div> : "Sign Up"}
          </button>
        </form>
        {error && (
          <p className="text-red-500 text-sm mt-4 animate__animated animate__shakeX">{error}</p>
        )}
        <p className="text-center text-gray-600 text-sm mt-4 animate__animated animate__fadeIn animate__delay-2s">
          Already have an account?{" "}
          <a href="/login" className="text-blue-500 hover:underline transition duration-200 ease-in-out">
            Log In
          </a>
        </p>
      </div>
